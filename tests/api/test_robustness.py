...
Cross-cutting concerns: response time, content-type, idempotency.
"""
import allure
import pytest
...
class TestRobustness:
    ...
    """
    RBS-004 (negative): Invalid integer id (string) still produces a defined response.
    Steps:
      1. GET /users/not-a-number
    ...
    def test_invalid_id_graceful_degradation(self, reqres, bad_id):
        response = reqres.session.get(f"{reqres.base_url}/users/{bad_id}", timeout=reqres.timeout)
        assert response.status_code < 500, f"Server should not 5xx on bad id, got {response.status_code}"
